...
- **Language change.** Poly now requires an explicit `fn main() ... end fn`
  entry point (or `async fn main()`). Top-level executable statements —
  `var`, `put`, loops, assignments, expressions — are rejected at check time
  with ``program has no `fn main() ... end fn` entry point: top-level
  executable statements are not allowed``. Declaration statements (functions,
  structs, enums, traits, impls, modules, consts, aliases, `use`, foreign
  blocks, `extern <target> fn` declarations) remain top-level.
- **Enforcement lives in the parser crate** (`poly-parser/src/entry_point.rs`,
  `require_explicit_main()` with 6 unit tests) and is wired into every
  transpile path: `check_program*` (checker), `Transpiler::transpile` (the
  unchecked path), and `poly_intermediate_representation::transpile` /
  `transpile_optimized`. A new negative-case CLI test
  (`test_missing_main_entry_point_is_rejected`) covers the error end-to-end.
- **The transpiler no longer auto-wraps.** Previously the IR generator
  synthesized `fn main()` from leftover top-level statements; generated Rust,
  C, asm, and JS outputs are unchanged for programs that already declared
  `fn main` (the backends spliced the declared body into the entry point
  before this change).
- **REPL adaptation.** Interactive sessions without an explicit `fn main`
  are wrapped in a synthetic one at transpile time, so line-by-line use is
  unchanged; `:tokens`/`:ast` still operate on the raw session source.
- **Repository sources migrated.** The 11 `.poly` files that relied on
  implicit top-level execution were wrapped in explicit `fn main() ... end fn`
  (`tests/{asm,c,js,target_flag,comprehensive_io,input,output}_*.poly`,
  `examples/{prime_numbers,poly_rust_hybrid,c_target_demo}.poly`,
  `benches/asm_benchmark.poly`); foreign blocks and extern declarations stay
  top-level. `examples/comprehensive_demo.poly` was reorganized so its
  declarations are hoisted and all executable statements live in one `fn
  main()` (its stale "Poly generates fn main() automatically" header comment
  was removed). `dclock_poly/dclock.poly` was wrapped the same way and its
  checked-in `src/main.rs` regenerated byte-identically with
  `poly --project build`.
- **Fixtures, snapshots, benches, playground.** Inline test sources in
  `poly-cli`, `poly-transpiler` (incl. the fuzz baseline), `poly-wasm`, and
  `poly-lsp` updated; 19 codegen snapshots regenerated (C emits
  `int main(void)`, JS `function main()`, asm `main:`, Rust `fn main()` as
  before); bench sources that measured the now-error path were fixed; the
  playground placeholder text now shows `fn main()`.
- **Documentation audit fixes (post-change fallout).** Doc examples that
  showed top-level statements were wrapped in `fn main()` or marked
  `poly fragment` (`README.md`, `POLY_SPEC_v2.md`, `POLY_ROADMAP_v2.md`,
...
- **Project audit.** A full feature audit was performed: feature-complete
  ...
  docs governance), partial (grammar doc lacks `js` target, CHANGELOG
  structure, C/asm/JS subset depth, Linux-only release artifacts), and
  not-implemented (`#cpp` backend, Phase 2 boundary stripping, multi-platform
  packaging, file I/O and typed input flags on non-Rust targets).
- Verification: 445/445 workspace tests, clippy `-D warnings` clean,
  `cargo fmt` clean, markdown convention check green (51 files), Poly doc
  audit green (46 blocks, 0 unmarked failures); positive case
  (`dclock.poly`, `hello_world.poly`) and negative case (top-level `put`
  rejected with exit code 1) verified with the release binary; REPL session
  wrap verified.
