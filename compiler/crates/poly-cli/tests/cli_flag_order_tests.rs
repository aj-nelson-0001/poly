...
#[test]
fn emit_flag_is_honored_when_the_file_comes_first() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn check_flag_is_honored_when_the_file_comes_first() -> Result<(), Box<dyn std::error::Error>> {
    let dir = unique_temp_dir("check")?;
    fs::create_dir_all(&dir)?;
    let source = dir.join("ok.poly");
    fs::write(&source, SIMPLE_PROGRAM)?;

    let output = poly().arg(&source).arg("--check").output()?;
    assert!(
        output.status.success(),
...
#[test]
fn target_js_default_build_runs_with_node() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = poly().arg("--target").arg("js").arg(&source).output()?;
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        output.status.success(),
...
#[test]
fn unknown_flags_are_rejected_after_the_file() -> Result<(), Box<dyn std::error::Error>> {
    let dir = unique_temp_dir("unknown")?;
    fs::create_dir_all(&dir)?;
    let source = dir.join("ok.poly");
    fs::write(&source, SIMPLE_PROGRAM)?;

    let output = poly().arg(&source).arg("--frobnicate").output()?;
    assert!(
        !output.status.success(),
...
#[test]
fn retired_statements_report_migration_suggestions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (snippet, expected_hint) in [
        ...
        fs::write(&source, snippet)?;
        let output = poly().arg("--check").arg(&source).output()?;
...
#[test]
fn function_local_consts_work_across_backends() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // C: same lowering.
    let output = poly().arg(&source).arg("--emit-c").output()?;
    assert!(
        output.status.success(),
        "c emit failed: {}",
        String::from_utf8_lossy(&output.stderr)
...
#[test]
fn dep_declarations_drive_project_manifest_and_check() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn dep_declarations_warn_on_non_rust_targets() -> Result<(), Box<dyn std::error::Error>> {
...
fn asm_backend_resolves_program_scope_structs_and_enums() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    for (tool, args) in [
        (
            ...
            vec![
                "-o",
                obj_path.to_str().ok_or("path should be UTF-8")?,
                asm_path.to_str().ok_or("path should be UTF-8")?,
            ],
        ),
        (
            "ld",
            vec![
                "-o",
                bin_path.to_str().ok_or("path should be UTF-8")?,
                obj_path.to_str().ok_or("path should be UTF-8")?,
            ],
        ),
    ] {
...
#[test]
fn match_expressions_work_on_c_and_js_targets() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output = poly().arg(&source).arg("--target").arg("js").output()?;
    assert!(
        output.status.success(),
        "js build failed: {}",
        String::from_utf8_lossy(&output.stderr)
...
fn asm_prints_negative_integers_correctly() -> Result<(), Box<dyn std::error::Error>> {
...
// The asm target emits Linux x86-64 syscall assembly; as/ld exist only where
// that output can assemble.
fn asm_passes_struct_returning_call_as_struct_argument() -> Result<(), Box<dyn std::error::Error>> {
...
#[test]
fn c_infers_struct_type_from_call_result_initializer() -> Result<(), Box<dyn std::error::Error>> {
